...
module rf
{
    ...
    export class Context3D
    {
        ...
        *  @variable must predefined in glsl
        */
        public setProgramConstantsFromMatrix(variable: string, matrix:Matrix3D, transposedMatrix: boolean = false): void
        {
            if(transposedMatrix)
                matrix.transpose();
        ...
        {
            GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, indexBuffer.glBuffer);
            GL.drawElements(GL.LINE_STRIP, numPoints < 0 ? indexBuffer.numIndices : numPoints, GL.UNSIGNED_SHORT, firstIndex * 2);
        }
